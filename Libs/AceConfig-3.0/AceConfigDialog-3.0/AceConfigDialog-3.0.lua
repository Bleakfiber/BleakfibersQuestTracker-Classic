...
function AceConfigDialog:AddToBlizOptions(appName, name, parent, ...)
	...
	if not BlizOptions[appName][key] then
		...
		if Settings and Settings.RegisterCanvasLayoutCategory then
			...
			if parent then
				local category = AceConfigDialog.BlizCategories[parent] or (Settings.GetCategory and Settings.GetCategory(parent))
				if not category then
					error(("The parent category '%s' was not found"):format(parent), 2)
				end
				local subcategory = Settings.RegisterCanvasLayoutSubcategory(category, group.frame, categoryName)
				categoryID = (subcategory.GetID and subcategory:GetID()) or subcategory.ID or subcategory.id
				AceConfigDialog.BlizCategories[categoryName] = subcategory
				if categoryID then
					AceConfigDialog.BlizCategories[categoryID] = subcategory
				end
				group:SetName(subcategory.ID or categoryName, parent)
			else
				local category = Settings.RegisterCanvasLayoutCategory(group.frame, categoryName)
				categoryID = (category.GetID and category:GetID()) or category.ID or category.id
				AceConfigDialog.BlizCategories[categoryName] = category
				AceConfigDialog.BlizCategories[appName] = category
				if categoryID then
					AceConfigDialog.BlizCategories[categoryID] = category
				end
				group:SetName(categoryName, parent)
				Settings.RegisterAddOnCategory(category)
			end
			group.frame.categoryID = categoryID
		else
			group:SetName(name or appName, parent)
			InterfaceOptions_AddCategory(group.frame)
			categoryID = group.frame.name
		end
		return group.frame, categoryID or group.frame.name
	else
